...
- **修改的地方：**
    ...
    2. **`MODEL_NAME`**：您将模型名称从 `deepseek-v3.1` 修正为了 API 支持的正确标识符，例如 `deepseek-chat`。
...
- **修改的地方：**
    ...
    1. **`SMTP_SERVER`**：您将这个 secret 的值修改为了您邮箱服务商提供的正确服务器地址（例如 `smtp.exmail.qq.com` 或 `smtp.gmail.com` 等）。
